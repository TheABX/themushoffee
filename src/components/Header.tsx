...
export default function Header() {
  return (
    <div className="sticky top-0 z-50">
      ...
      <header className="bg-ink/95 backdrop-blur border-b border-cream/10">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 flex items-center justify-between h-16 sm:h-20">
          ...
          <Link
            href="/#notify"
            className="inline-flex items-center whitespace-nowrap rounded-full bg-purple px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-white hover:bg-purple-dark transition-colors shrink-0"
          >
            Join Waitlist
          </Link>
        </div>
        <nav className="md:hidden flex items-center gap-5 overflow-x-auto px-4 pb-3 text-xs font-medium text-cream/70">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
